...
var PORTFOLIO = 'https://papi.binance.com';
...
var publicCall = function publicCall(_ref) {
  ...
  return function (path, data) {
    ...
    return sendResult(fetch("".concat(path.includes('/papi') ? endpoints.portfolio : path.includes('/fapi') || path.includes('/futures') ? endpoints.futures : path.includes('/dapi') ? endpoints.delivery : endpoints.base).concat(path).concat(makeQueryString(data)), _objectSpread({
      method: method,
      json: true,
      headers: headers
    }, proxy ? {
...
var privateCall = function privateCall(_ref3) {
  ...
  return function (path) {
    ...
    return (data && data.useServerTime ? pubCall('/api/v3/time').then(function (r) {
      ...
      return sendResult(fetch("".concat(path.includes('/papi') ? endpoints.portfolio : path.includes('/fapi') || path.includes('/futures') ? endpoints.futures : path.includes('/dapi') ? endpoints.delivery : endpoints.base).concat(path).concat(noData ? '' : makeQueryString(newData)), _objectSpread({
        method: method,
        headers: {
          'X-MBX-APIKEY': apiKey
        },
...
var _default = function _default(opts) {
  ...
  return {
    ...
    portfolioGetDataStream: function portfolioGetDataStream() {
      return privCall('/papi/v1/listenKey', null, 'POST', true);
    },
    portfolioKeepDataStream: function portfolioKeepDataStream(payload) {
      return privCall('/papi/v1/listenKey', payload, 'PUT', false, true);
    },
    portfolioCloseDataStream: function portfolioCloseDataStream(payload) {
      return privCall('/papi/v1/listenKey', payload, 'DELETE', false, true);
    },
    ...
    portfolioPing: function portfolioPing() {
      return pubCall('/papi/v1/ping').then(function () {
        return true;
      });
    },
    portfolioUMOrder: function portfolioUMOrder(payload) {
      return _order(privCall, payload, '/papi/v1/um/order');
    },
    portfolioUMCancelOrder: function portfolioUMCancelOrder(payload) {
      return privCall('/papi/v1/um/order', payload, 'DELETE');
    },
    portfolioUMCancelAllOpenOrders: function portfolioUMCancelAllOpenOrders(payload) {
      return privCall('/papi/v1/um/allOpenOrders', payload, 'DELETE');
    },
    portfolioUMPositionRisk: function portfolioUMPositionRisk(payload) {
      return privCall('/papi/v1/um/positionRisk', payload);
    },
